...
const component: JSXMapSerializer = {
	...
	heading3: ({ children }) => (
		<Heading
			as='h3'
			size='xs'
			className='mb-3 text-xl font-extrabold md:mb-4 md:text-2xl'>
			{children}
		</Heading>
	),
...
const Process: FC<ProcessProps> = ({ slice }) => {
	...
	return (
		<section className='font-redhat text-brand-dark-blue px-[5%] py-16 md:py-24 lg:py-28 border-b-4 border-black relative overflow-hidden'>
			<div className='container overscroll-none'>
				...
				<div className='grid place-items-center gap-x-8 gap-y-8 sm:grid-cols-2 md:gap-y-16 lg:mx-auto lg:grid-cols-none'>
					<div className='w-full grid-cols-1 gap-x-20 gap-y-8 md:gap-y-16 grid lg:hidden'>
						{steps.slice(0, 2).map((step, index) => (
							<div key={index} className='flex flex-col neobrutalist-shadow-left items-center text-center p-3'>
								{step && (
									<StepItem
										icon={step.icon}
										text={step.text}
					...
					<div className='order-last w-full sm:col-span-2 lg:order-none lg:col-span-none lg:relative'>
						{steps.map((step, index) => {
							...
							return (
								<div key={index} className={`hidden lg:block absolute neobrutalist-shadow-left ${positions[index].className} w-[338px] h-[230px]`}>
									{step && (
										<StepItem
											icon={step.icon}
											text={step.text}
